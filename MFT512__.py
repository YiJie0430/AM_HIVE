...
def W11(parent):
    try:
        ...
        if not htx.IsConnect(dut_ip,60): raise Except("DUT IP %s never connect\n"%dut_ip)   
        term_TCP = htx.TCPClient("127.0.0.1",6432)  
        ...
        CheckNartReady(parent,'2G',pwr_check,log)
        term_TCP << '%s'%mac
