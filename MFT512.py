...
################### 20180920 add: certificate md5 check
####################20180920 add: power check algorithm
def CheckNrm(func):
    def checkdata(*args):
        result = 1
        rep = str()
        ...
        if not args[5]:
            ...
            source_md5 = os.popen('md5sums {}\\certificate\\*'.format(os.getcwd())).read()
            args[0].SendMessage('\nSource File MD5:\n{}'.format(source_md5), args[4])
            for item in list(data[1].keys()):
                if data[1][item] not in source_md5:
                    raise Except('\n{} Certificate failed: {}'.format(data[1][item]))
                else:
                    args[0].SendMessage('\n{} MD5 check: {}'.format(item, data[1][item]), args[4])
...
def CheckBootUp(func):
    def checkstatus(*args):
        ...
        data = func(args[0], term, args[2], args[3], args[4], args[5])
        if data[0]:
            args[0].SendMessage( "\nNrmSetup Test time: %3.2f (sec)\n"%((time.time() - test_time)),args[4])
            args[0].SendMessage( "---------------------------------------------------------------------------\n",args[4])
            return term
        else:
...
@CheckBootUp
@CheckNrm
def NrmSetup(parent,term,mac,sn,log,pwr_check):
    data=str()
    cer_md5 = dict()
    ...
    # read the certificate of the board to (type(dict))
    data = lWaitCmdTerm(term, 'md5sum /var/certs/*', '#', 5)
    md5 = re.findall(r'(?<=)(.*)(?<=\s\s/)', data, re.M)
    for idx, item in enumerate(['ca.pem', 'client.pem', 'client_dec.key']):
        idx*=2
        if len(md5[idx].split('/')[0].strip()) != 32:
            raise('\nMD5 Parsing error: {}\n'.format(md5[idx].split('/')[0].strip()))
        cer_md5.update({item: md5[idx].split('/')[0].strip()})
    return (report, cer_md5)
    

def ProcessMonitor(func):
    def Monitor(*args):
        wait_str = {'2G':['nart','cd / && ./etc/init.d/art stop'],
                    '5G':['Qcmbr','cd / && ./etc/init.d/qcmbr stop']}
        for i in wait_str.keys():
            process = lWaitCmdTerm(args[1], 'ps | grep {}'.format(wait_str[i][0]), '#', 5)
            if '/usr/sbin' in process:
                lWaitCmdTerm(args[1], wait_str[i][1], '#', 5)
                lWaitCmdTerm(args[1], 'pgrep {} | xargs kill'.format(wait_str[i][0]), '#', 5)
        func(args[0], args[1], args[2], args[3], args[4])
    return Monitor

################### add  in 20160121  by TDL #######################################    
@ProcessMonitor
def CheckNartReady(parent,term,interface,pwr_check,log):
    test_time = time.time()
    wait_str = {'2G':['nart.out', 'cd / && ./etc/init.d/art start'], '5G':['Qcmbr','cd / && ./etc/init.d/qcmbr start']}
    parent.SendMessage("Start up %s by %s\n"%(interface,wait_str[interface][1]),log)
    lWaitCmdTerm(term, wait_str[interface][1], '#', 10)
    process = lWaitCmdTerm(term, 'ps | grep {}'.format(wait_str[interface][0]), '#', 5)
    if '/usr/sbin' not in process:
        raise Except('{} initial fail: {}'.format(interface, process))
        ...
        if (test_2g & test_5g):
            ...
            if "Finish" in NI_result[1]:              
                ...
                if not result_: 
                    ...
                    if not pwr_check:
                        CheckNartReady(parent,term,'5G',pwr_check,log)
                    term_TCP << 'Run'
                    data = term_TCP.wait("OK",10)[-1]
